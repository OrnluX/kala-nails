    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(!empty($data->username)  && !empty($data->password)) {
            ...
            if ($user && password_verify($password, $user['password'])) { //password_verify() verifica si el password coincide con el hash almacenado
                ...
                $randomVariant = bin2hex(random_bytes(16)); //Variante random para concatenar a la llave secreta en esta instancia específica
                $randomKey = $secretKey . $randomVariant;
                $token = JWT::encode($tokenPayload, $randomKey, 'HS256'); //Almacena el token encriptado con el algoritmo HS256
